...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        ...
        zero_count = len(data[data['class'] == 0])
        one_count = len(data[data['class'] == 1])
...
def get_scatter_chart(entered_site, payload_range):
    if entered_site == 'ALL':
        ...
        # return the outcomes piechart for a selected site
    filtered_df = data[(data['Payload Mass (kg)'] >= payload_range[0])
                       & (data['Payload Mass (kg)'] <= payload_range[1])]
    fig = px.scatter(filtered_df, x='Payload Mass (kg)', y='class',
                     color="Booster Version Category",
                     title=f"Success rate by payload and booster version at site: {entered_site}")
